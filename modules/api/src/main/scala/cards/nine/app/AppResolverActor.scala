...
class AppResolverActor[F[_]](interpreter: F ~> Task, log: LoggingAdapter)(
    implicit process: ApplicationProcesses[F])
    extends Actor {
  ...
  private[this] def showAppResolutionInfo(result: Throwable \/ Result[ResolvePendingStats]) =
    result match {
      ...
      case \/-(Right(stats)) ⇒
        log.info(
          s"The server resolved ${stats.resolved} packages from the pending queue to an App")
        log.info(
          s"The server resolved ${stats.errors} pending packages not to exist in the App Store")
