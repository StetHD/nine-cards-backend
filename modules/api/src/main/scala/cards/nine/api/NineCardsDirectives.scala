...
class NineCardsDirectives(
    implicit accountProcesses: AccountProcesses[NineCardsServices],
  ...
  class AppCuratorAuthenticator(editors: Map[String, String])
      extends UserPassAuthenticator[String] {
  ...
  val generateSessionToken: Directive1[SessionToken] = provide(
