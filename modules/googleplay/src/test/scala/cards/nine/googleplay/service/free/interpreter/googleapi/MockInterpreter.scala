...
trait InterpreterServer[F[_]] {
  ...
  def getDetailsList(
      ...
      marketAuth: MarketCredentials): F[List[Failure Either FullCard]]
  def getBulkDetails(
      packageNames: List[Package],
      marketAuth: MarketCredentials): F[Failure Either List[BasicCard]]
  def recommendationsByApps(
      request: RecommendByAppsRequest,
      auth: MarketCredentials): F[List[Package]]
  def recommendationsByCategory(
      request: RecommendByCategoryRequest,
      auth: MarketCredentials): F[InfoError Either List[Package]]
  def searchApps(
      request: SearchAppsRequest,
      auth: MarketCredentials): F[Failure Either List[Package]]
}
