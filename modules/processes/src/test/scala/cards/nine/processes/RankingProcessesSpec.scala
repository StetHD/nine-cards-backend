...
trait RankingsProcessesSpecification
  ...
  trait BasicScope extends Scope {
    ...
    implicit val analyticsServices: GoogleAnalytics.Services[NineCardsApp.T] =
      mock[GoogleAnalytics.Services[NineCardsApp.T]]
    implicit val countryServices: Country.Services[NineCardsApp.T] =
      mock[Country.Services[NineCardsApp.T]]
    implicit val rankingServices: Ranking.Services[NineCardsApp.T] =
      mock[Ranking.Services[NineCardsApp.T]]

    val rankingProcesses = RankingProcesses.processes[NineCardsApp.T]
...
class RankingsProcessesSpec extends RankingsProcessesSpecification {
  ...
  "getRanking" should {
    "give a valid ranking if the cache has info for the given scope" in new BasicScope {
      ...
      rankingProcesses.getRanking(scope).exec[Id] must beRight[Get.Response].which {
        response ⇒
    ...
    "give a RankingNotFound error if the cache has no info for the given scope" in new BasicScope {
      ...
      rankingProcesses.getRanking(scope).exec[Id] must beLeft[NineCardsError]
    }
  }
  ...
  "reloadRanking" should {
    "give a good answer" in new BasicScope {
      ...
      response.exec[Id] must beRight(Reload.Response())
    }
  }
  ...
  "getRankedApps" should {
    "return an empty response if no device apps are given" in new BasicScope {
      ...
      response.exec[Id] must beRight[List[RankedAppsByCategory]](Nil)
    }
    "return all the device apps as ranked if there is ranking info for them" in new BasicScope {
      ...
      response.exec[Id] must beRight[List[RankedAppsByCategory]].which { r ⇒
  ...
  "getRankedWidgets" should {
    "return an empty response if no apps are given" in new BasicScope {
      ...
      response.exec[Id] must beRight[List[RankedWidgetsByMoment]](Nil)
    }
    "return all the widgets as ranked if there is ranking info for them" in new BasicScope {
      ...
      response.exec[Id] must beRight[List[RankedWidgetsByMoment]].which { r ⇒
