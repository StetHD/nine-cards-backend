...
class DatabaseTransactor[M[_]: Catchable: Capture](config: DatabaseConfiguration)(
    implicit MM: Monad[M]) {
...
object DatabaseTransactor {
  ...
  implicit val transactor: HikariTransactor[Task] =
    new DatabaseTransactor[Task](nineCardsConfiguration.db).hikariTransactor.unsafePerformSync
}
