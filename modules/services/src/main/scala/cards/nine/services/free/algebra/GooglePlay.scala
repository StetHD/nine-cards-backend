...
object GooglePlay {
  ...
  case class Resolve(packageName: Package, auth: MarketCredentials) extends Ops[Result[FullCard]]
  ...
  class Services[F[_]](implicit I: Ops :<: F) {
    ...
    def recommendByCategory(
      ...
      NineCardsService(
        RecommendationsByCategory(category, priceFilter, excludesPackages, limit, auth))
