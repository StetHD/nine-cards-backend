...
class Interpreters(implicit A: ApplicativeError[Task, Throwable], T: Transactor[Task]) {
  ...
  val analyticsInterpreter: (GoogleAnalytics.Ops ~> Task) =
    AnalyticsServices.services(nineCardsConfiguration.google.analytics)

  val collectionInterpreter: (SharedCollection.Ops ~> Task) =
    CollectionServices.services.andThen(connectionIO2Task)

  val countryInterpreter: (Country.Ops ~> Task) =
    CountryServices.services.andThen(connectionIO2Task)

  val firebaseInterpreter: (Firebase.Ops ~> Task) =
    FirebaseServices.services(nineCardsConfiguration.google.firebase)

  val googleApiInterpreter: (GoogleApi.Ops ~> Task) =
    GoogleApiServices.services(nineCardsConfiguration.google.api)
  ...
  val subscriptionInterpreter: (Subscription.Ops ~> Task) =
