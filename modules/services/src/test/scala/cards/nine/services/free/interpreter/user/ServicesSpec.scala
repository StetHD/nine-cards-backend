    ...
    def apply[A](
    ...
    )(check: ⇒ MatchResult[A]) = {
      {
        for {
          ...
          u ← insertItem(User.Queries.insert, userData.toTuple)
          i ← insertItem(
            Installation.Queries.insert,
            (u, Option(deviceToken.value), androidId.value))
          c ← insertItem(
            SharedCollection.Queries.insert,
            collectionData.copy(userId = Option(u)).toTuple)
  ...
  "addUser" should {
    "new users can be created" in {
      prop { userData: UserData ⇒
        WithEmptyDatabase {
          val insertedUser = userPersistenceServices
            .addUser(
    ...
    "return an user if there is an user with the given email in the database" in {
      prop { userData: UserData ⇒
        WithData(userData) { id ⇒
          ...
          user should beRight[User].which { user ⇒
            user.id must_== id
  ...
  "getUserBySessionToken" should {
    "return an UserNotFound error if the table is empty" in {
      prop { (email: Email, sessionToken: SessionToken) ⇒
        WithEmptyDatabase {
          val user = userPersistenceServices
    ...
    "return an UserNotFound error if there isn't any user with the given sessionToken in the database" in {
      prop { userData: UserData ⇒
        WithData(userData) { id ⇒
          ...
          val user =
            userPersistenceServices.getUserBySessionToken(wrongSessionToken).transactAndRun

          user should beLeft(
            UserNotFound(s"User with sessionToken ${wrongSessionToken.value} not found"))
        }
      }
    }
  }
    ...
    "new installation can be created" in {
      prop { (androidId: AndroidId, userData: UserData) ⇒
        WithData(userData) { userId ⇒
          val insertedInstallation = userPersistenceServices
            .createInstallation(
              userId = userId,
              deviceToken = None,
              androidId = androidId
  ...
  "getInstallationByUserAndAndroidId" should {
    "return an InstallationNotFound error if the table is empty" in {
      prop { (androidId: AndroidId, userId: Long) ⇒
        WithEmptyDatabase {
          val installation = userPersistenceServices
    ...
    "installations can be queried by their userId and androidId" in {
      prop { (androidId: AndroidId, userData: UserData) ⇒
        WithData(userData, androidId) { (userId, installationId) ⇒
          ...
          installation should beRight[Installation].which { install ⇒
            install.id must_== installationId
          }
        }
      }
    }
    "return an InstallationNotFound error if there isn't any installation with the given userId " +
      ...
      prop { (androidId: AndroidId, userData: UserData) ⇒
        WithData(userData, androidId) { (userId, installationId) ⇒
          ...
          installation must beLeft(
            InstallationNotFound(s"Installation for android id ${wrongAndroidId.value} not found"))
        }
      }
    }
  }
